...
1：赞助者（>=20 RMB/年）以及翻译贡献者，Dism++将针对这部分用户额外提供Windows Update扩展数据库支持（覆盖Vista~Windows 10所有操作系统，包含服务器），并且拥有更快的更新速度。具体可以凭借支付订单通过邮件申请（mingkuang@.live.com）。
2：赞助者（仅12个月内）、提供Dism++规则的用户（在Data.xml可以搜索到名字的人，并且最新规则日期在12个月内）、BUG反馈者（readme中可以搜索到的，并且最新BUG提交日期在12月内。），提供 `1次` Dism++文档制定（比如说，某人想了解WIM与ESD的区别），Dism++将提供文档帮助。
...
> 赞助后如果反悔，可以将交易流水号以及交易时间发送到邮箱（mingkuang@live.com），我将通过原路径退回赞助金额（警告：不受理超过12个月前的交易）。
